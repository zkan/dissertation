...
Several well-known algorithms for shadow detection already exist. Most of the
...
the normalized RGB color space \shortcite{finlayson98colour}.  Some research
work thus utilizes the normalized RGB space in the background subtraction and
shadow removal algorithm
\shortcite{Malena00tracking,elgammal02background,hong03background}.

\shortciteA{mikic00shadow} observe that in the normalized RGB color space,
...
\shortciteA{salvador04shadow} propose a new method for detecting cast shadows.
...
\shortciteA{havasi06geometric} illustrate that color-based methods work well
...
One well-known problem with the normalized RGB space is that normalization of
pixels with low intensity results in unstable chromatic components
\shortcite{kender76color}. \shortciteA{cucchiara01shadow} and
\shortciteA{chen08shadow} propose a HSV color-based method to distinguish
...
Similarly, the probability of measurement given its assignment for 
